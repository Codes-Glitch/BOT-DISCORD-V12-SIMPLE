...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    let refix = await db.fetch(`Prefix_${message.guild.id}`);
    if (!refix) refix = Default_Prefix;
    ...
    if (NewPrefix === refix) return message.channel.send("Given Prefix Is The Current Prefix!");
